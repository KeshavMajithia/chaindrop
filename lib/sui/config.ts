...
try {
  ...
  WalletAccount = dappKitModule.WalletAccount
  getFullnodeUrl = suiClientModule.getFullnodeUrl
} catch (error) {
  console.warn('Sui packages not found, using fallback implementations')
  // Fallback implementations for when packages aren't installed
  SuiClient = class MockSuiClient {
    constructor(url: string) {
      console.warn('Using mock SuiClient')
    }
  }
  WalletAccount = class MockWalletAccount {}
